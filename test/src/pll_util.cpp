...
TEST(pll_util, set_unique_clv_indices) {
  ...
  for (auto x : tip_nodes) index_list.push_back(x->clv_index);
...
static void test_newick_string(std::string const& tree_file, std::string const& valid) {
  ...
  pll_utree_traverse(get_root(tree), PLL_TREE_TRAVERSE_POSTORDER, cb_set_branchlengths_one,
                     &travbuffer[0], &traversal_size);
...
TEST(pll_util, get_numbered_newick_string) {
  test_newick_string(env->tree_file,
                     "(A:1.00{0},(B:1.00{1},(C:1.00{2},(D:1.00{3},(E:1.00{4},(F:1.00{5},G:1.00{6}):"
                     "1.00{7}):1.00{8}):1.00{9}):1.00{10}):1.00{11},H:1.00{12});");

  test_newick_string(
      env->data_dir + "ref_innerlabels.tre",
      "(A:1.00{0},(B:1.00{1},(C:1.00{2},(D:1.00{3},(E:1.00{4},(F:1.00{5},G:1.00{6})FG:1.00{7})EFG:"
      "1.00{8})DEFG:1.00{9})CDEFG:1.00{10})BCDEFG:1.00{11},H:1.00{12})ABCDEFGH;");
}
...
TEST(pll_util, get_numbered_newick_string_rooted) {
  test_rooted_preserve(env->tree_file_rooted,
                       "((((G:1.01{0},H:1.08{1}):0.01{2},A:1.34{3}):1.00{4},B:1.66{5}):1.01{6},(C:"
                       "1.08{7},D:1.26{8}):1.12{9});");

  test_rooted_preserve(env->tree_file_rooted_2,
                       "(A:1.34{0},((B:1.66{1},(C:1.08{2},D:1.26{3}):1.12{4}):1.00{5},(G:1.01{6},H:"
                       "1.08{7}):1.90{8}):0.01{9});");

  test_rooted_preserve(env->tree_file_rooted_3,
                       "(((A:1.34{0},(B:1.66{1},(C:1.08{2},D:1.26{3}):1.12{4}):1.00{5}):1.01{6},G:"
                       "1.08{7}):1.90{8},H:0.01{9});");

  test_rooted_preserve(env->data_dir + "ref_rooted_innerlabels.tre",
                       "((((G:1.01{0},H:1.08{1})GH:0.01{2},A:1.34{3})GHA:1.00{4},B:1.66{5})GHAB:1."
                       "01{6},(C:1.08{7},D:1.26{8})CD:1.12{9})GHABCD;");

  test_rooted_preserve(env->data_dir + "ref_rooted_2_innerlabels.tre",
                       "(A:1.34{0},((B:1.66{1},(C:1.08{2},D:1.26{3})CD:1.12{4})BCD:1.00{5},(G:1.01{"
                       "6},H:1.08{7})GH:1.90{8})BCDGH:0.01{9})ABCDGH;");

  test_rooted_preserve(env->data_dir + "ref_rooted_3_innerlabels.tre",
                       "(((A:1.34{0},(B:1.66{1},(C:1.08{2},D:1.26{3})CD:1.12{4})BCD:1.00{5})ABCD:1."
                       "01{6},G:1.08{7})ABCDG:1.90{8},H:0.01{9})ABCDGH;");
}
...
TEST(pll_util, shift_partition_focus_logtest) {
  ...
  // tests
  double full_logl = pll_compute_edge_loglikelihood(part, 0, PLL_SCALE_BUFFER_NONE, 3, 0, 0,
  ...
  double ranged_logl = pll_compute_edge_loglikelihood(part, 0, PLL_SCALE_BUFFER_NONE, 3, 0, 0,
                                                      param_indices, nullptr);
  ...
  double false_logl = pll_compute_edge_loglikelihood(part, 0, PLL_SCALE_BUFFER_NONE, 3, 0, 0,
                                                     param_indices, nullptr);
  ...
  false_logl += pll_compute_edge_loglikelihood(part, 0, PLL_SCALE_BUFFER_NONE, 3, 0, 0,
                                               param_indices, nullptr);
  ...
  pll_compute_edge_loglikelihood(part, 0, PLL_SCALE_BUFFER_NONE, 3, 0, 0, param_indices, nullptr);
  // printf("fmr+left:\t %.40f\n", false_logl);
