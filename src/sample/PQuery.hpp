...
public:
  ...
  PQuery(const PQuery<Slim_Placement>& other)
      : sequence_id_(other.sequence_id()), placements_(other.size()) {
    const auto size = other.size();
    for (size_t i = 0; i < size; ++i) {
      placements_[i] = Placement(other.at(i));
    }
  }
  PQuery(const seqid_type seq_id, const std::string& header)
      : sequence_id_(seq_id), header_(header) {}
  PQuery(const seqid_type seq_id) : sequence_id_(seq_id) {}
  ~PQuery() = default;
  ...
  inline std::vector<value_type>& data() { return placements_; }
  ...
  void serialize(Archive& ar) {
    ar(sequence_id_, header_, placements_);
  }
