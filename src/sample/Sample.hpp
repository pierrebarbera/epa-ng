...
public:
  ...
  template <typename T = Placement_Type,
            typename = std::enable_if_t<std::is_same<T, Placement>::value> >
  Sample(Sample<Slim_Placement> const& other) : pquerys_(other.size()), newick_(other.newick()) {
  ...
  Sample(size_t const size, size_t const depth) : Sample(size) {
    for (size_t i = 0; i < pquerys_.size(); ++i) {
      pquerys_[i].resize(depth);
    }
  }

  explicit Sample(std::string const& newick) : newick_(newick) {}
  ~Sample() = default;
  ...
  void insert(InputIt first, InputIt last) {
    pquerys_.insert(pquerys_.end(), first, last);
  }

  template <typename... Args>
  void add_placement(const size_t seq_id, const std::string& label, Args&&... args) {
  ...
  size_t add_pquery(const size_t seq_id, const std::string& label) {
