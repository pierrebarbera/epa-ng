...
class Lookup_Store {
  /**
   * NOTE TO FUTURE DEVS:
   * This class has gotten a bit convoluted, so where is a brief overview of the various maps and
   * tables:
   *
   * store_: vector of matrices, one per branch in the ref tree
   * <matrix in store>-> lookup_matrix: stores one CLV per character suitable for the model (ACGTVH-
   * etc.) char_map_: set of chars for which a lookup_matrix is done (see util/maps.hpp)
...
public:
  ...
  std::mutex& get_mutex(const size_t branch_id) { return branch_[branch_id]; }

  bool has_branch(const size_t branch_id) { return store_[branch_id].size() != 0; }

  lookup_type& operator[](const size_t branch_id) { return store_[branch_id]; }

  unsigned char char_map(const size_t i) {
    if (i >= char_map_size_) {
      throw std::runtime_error{std::string("char_map access out of bounds! i =") +
                               std::to_string(i)};
    }
  ...
  double sum_precomputed_sitelk(const size_t branch_id, const std::string& seq,
