                                          ...
                                          double const tolerance )
{
  ...
  while( smoothings ) {
    auto const old_blonode_length = blo_node->length;
    auto const old_pendant_length = score_node->length;
                                       ...
                                       int* smoothings )
{
  ...
  cur_logl = -1 * pllmod_opt_optimize_branch_lengths_iterative( partition, root, &param_indices[ 0 ], PLLMOD_OPT_MIN_BRANCH_LEN, PLLMOD_OPT_MAX_BRANCH_LEN, OPT_BRANCH_EPSILON, *smoothings,
               ...
               bool const opt_model )
{
  ...
  auto const rates_size = model.subst_rates( 0 ).size();
...
void compute_and_set_empirical_frequencies( pll_partition_t* partition, raxml::Model& model )
{
  ...
  for( size_t i = 0; i < model.num_submodels(); ++i ) {
    model.base_freqs( i, std::vector< double >( partition->frequencies[ i ], partition->frequencies[ i ] + partition->states ) );
  }
