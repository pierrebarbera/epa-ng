...
{
  if( fptr )
    pll_fasta_close( fptr );
}

static void set_missing_branch_lengths_recursive( pll_unode_t* tree, double const length )
{
  if( tree ) {
...
void set_missing_branch_lengths( pll_utree_t* tree, double const length )
{
...
static void set_branch_lengths_recursive( pll_unode_t* tree, double const length )
{
  if( tree ) {
    tree->length = length;
...
void set_branch_lengths( pll_utree_t* tree, double const length )
{
  auto const root = get_root( tree );
...
static void set_unique_clv_indices_recursive( pll_unode_t* tree, int const num_tip_nodes )
{
...
void set_unique_clv_indices( pll_unode_t* const tree, int const num_tip_nodes )
{
...
void shift_partition_focus( pll_partition_t* partition,
                            int const offset,
                            unsigned int const span )
{
  auto const clv_size  = static_cast< int >( partition->rate_cats * partition->states_padded );
  auto const num_tips  = partition->tips;
  auto const max_index = num_tips + partition->clv_buffers;
