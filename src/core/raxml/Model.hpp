...
public:
  explicit SubstitutionModel(const pllmod_subst_model_t& sm) : _states(sm.states), _name(sm.name) {
    if (sm.freqs) _base_freqs.assign(sm.freqs, sm.freqs + sm.states);
    if (sm.rates) _subst_rates.assign(sm.rates, sm.rates + sm.states * (sm.states - 1) / 2);
    if (sm.rate_sym) _rate_sym.assign(sm.rate_sym, sm.rate_sym + sm.states * (sm.states - 1) / 2);
    if (sm.freq_sym) _freq_sym.assign(sm.freq_sym, sm.freq_sym + sm.states);
  };
  ...
  doubleVector uniq_subst_rates() const {
    if (!_rate_sym.empty()) {
      ...
      for (size_t i = 0; i < _subst_rates.size(); ++i) uniq_rates[_rate_sym[i]] = _subst_rates[i];
      return uniq_rates;
    } else
      return _subst_rates;
  };
  ...
  void subst_rates(const doubleVector& v) {
    if (v.size() != num_rates()) throw std::invalid_argument("Invalid size of subst_rates vector!");
  ...
  void uniq_subst_rates(const doubleVector& v) {
    if (!_rate_sym.empty()) {
      ...
      for (size_t i = 0; i < _subst_rates.size(); ++i) _subst_rates[i] = v[_rate_sym[i]];
    } else
      subst_rates(v);
  };
...
public:
  ...
  void base_freqs(const doubleVector& value) {
    for (SubstitutionModel& s : _submodels) s.base_freqs(value);
  }
  void subst_rates(const doubleVector& value) {
    for (SubstitutionModel& s : _submodels) s.subst_rates(value);
  }
  ...
  bool empirical_base_freqs() {
    return _param_mode.at(PLLMOD_OPT_PARAM_FREQUENCIES) == ParamValue::empirical;
  }
