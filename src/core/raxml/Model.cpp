...
void Model::autodetect_data_type( const std::string& model_name )
{
  if( _data_type == DataType::autodetect ) {
    ...
    else if( pllmod_util_model_exists_protein( model_name.c_str() ) || pllmod_util_model_exists_protmix( model_name.c_str() ) ) {
...
void Model::init_model_opts( const std::string& model_opts, const pllmod_mixture_model_t& mix_model )
{
  ...
  _param_mode[ PLLMOD_OPT_PARAM_FREQUENCIES ] = mix_model.models[ 0 ]->freqs ? ParamValue::model : ParamValue::ML;

  _param_mode[ PLLMOD_OPT_PARAM_SUBST_RATES ] = mix_model.models[ 0 ]->rates ? ParamValue::model : ParamValue::ML;
  ...
  try {
    ...
    if( read_param( ss, user_srates ) ) {
      ...
      if( user_srates.size() != num_uniq_rates ) {
        throw runtime_error( "Invalid number of substitution rates specified: " + std::to_string( user_srates.size() ) + " (expected: " + std::to_string( num_uniq_rates ) + ")\n" );
      }

      // normalize the rates
      auto last_rate = smodel.rate_sym().empty() ? user_srates.back() : user_srates[ smodel.rate_sym().back() ];
        ...
        case 'U': {
          ...
          if( read_param( ss, user_freqs ) ) {
            if( user_freqs.size() != _num_states ) {
              throw runtime_error( "Invalid number of user frequencies specified: " + std::to_string( user_freqs.size() ) + "\n" + "Number of frequencies must be equal to the number of states: " + std::to_string( _num_states ) + "\n" );
            }
      ...
      try {
        ...
        if( read_param( ss, v ) ) {
          if( v.size() != _num_ratecats ) {
            throw runtime_error( "Invalid number of free rates specified: " + std::to_string( v.size() ) + " (expected: " + std::to_string( _num_ratecats ) + ")\n" );
          }
          ...
          if( read_param( ss, v ) ) {
            if( v.size() != _num_ratecats ) {
              throw runtime_error( "Invalid number of rate weights specified: " + std::to_string( v.size() ) + " (expected: " + std::to_string( _num_ratecats ) + ")\n" );
            }
...
{
  if( model.num_states() == partition->states && model.num_submodels() == partition->rate_matrices ) {
    model.pinv( partition->prop_invar[ 0 ] );
    for( size_t i = 0; i < model.num_submodels(); ++i ) {
      model.base_freqs( i, doubleVector( partition->frequencies[ i ], partition->frequencies[ i ] + partition->states ) );

      size_t n_subst_rates = pllmod_util_subst_rate_count( partition->states );
      model.subst_rates( i, doubleVector( partition->subst_params[ i ], partition->subst_params[ i ] + n_subst_rates ) );
    }
...
static string get_ratehet_mode_str( const Model& m )
{
  if( m.num_ratecats() == 1 )
    ...
    return ( m.ratehet_mode() == PLLMOD_UTIL_MIXTYPE_GAMMA ) ? "GAMMA" : ( m.ratehet_mode() == PLLMOD_UTIL_MIXTYPE_FREE ) ? "FREE" : "FIXED";
}
...
ostringstream& raxml::operator<<( ostringstream& stream, const Model& m )
{
  ...
  if( m.num_ratecats() > 1 ) {
    stream << " (" << m.num_ratecats() << " cats, " << ( m.gamma_mode() == PLL_GAMMA_RATES_MEDIAN ? "median" : "mean" ) << ")";
  ...
  if( m.param_mode( PLLMOD_OPT_PARAM_PINV ) != ParamValue::undefined )
    stream << "   P-inv (" << get_param_mode_str( m.param_mode( PLLMOD_OPT_PARAM_PINV ) ) << "): " << m.pinv() << endl;
