...
public:
   ...
   */
  Work(std::pair<key_type, key_type>&& branch_range,
       std::pair<value_type, value_type>&& seq_range) {
  ...
  size_t size() const {
    return std::accumulate(
        work_set_.begin(), work_set_.end(), 0,
...
public:
  ...
  WorkIterator(WorkType const& target, bool is_end)
      : map_it(std::begin(target)), end_it(std::end(target)), vec_it() {
    if (is_end) {
      map_it = end_it;
    }
  ...
  element_type operator*() { return {map_it->first, *vec_it}; }
  ...
  size_t current_branch_id() { return map_it->first; }

  size_t current_sequence_id() { return *vec_it; }
...
inline void Work::add(Work_Pair& it) { add(it.branch_id, it.sequence_id); }

inline Work::const_iterator Work::begin() const { return WorkIterator(work_set_, false); }

inline Work::const_iterator Work::end() const { return WorkIterator(work_set_, true); }