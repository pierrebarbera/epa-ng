...
public:
  ...
  Timer(duration&& d) { ts_.push_back(d); }
  ...
  void pause() { pause_start_ = clock::now(); }
  ...
  double sum() const { return this->sum_duration().count(); }
  ...
  duration avg_duration() const { return this->sum_duration() / ts_.size(); }

  double average() const { return this->sum() / ts_.size(); }
