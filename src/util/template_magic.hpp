...
constexpr size_t array_size(T (&)[S]) noexcept {
  return S;
}
...
template <class... Ts>
struct leading_types : leading_types_base<std::make_index_sequence<sizeof...(Ts) - 1u>, Ts...> {};
 ...
 */
template <std::size_t I = 0, class FuncT, class... Tp>
inline typename std::enable_if<I == sizeof...(Tp) - 1u, void>::type for_each_pair(
    std::tuple<Tp...>&, FuncT)  // Unused arguments are given no names.
{}

template <std::size_t I = 0, class FuncT, class... Tp>
    inline typename std::enable_if <
    I<sizeof...(Tp) - 1u, void>::type for_each_pair(std::tuple<Tp...>& t, FuncT f) {
...
struct token_types_base<std::index_sequence<I...>, StageTuple> {
  using types = typename std::tuple<typename std::tuple_element<0u, StageTuple>::type::in_type,
