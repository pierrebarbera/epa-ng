...
static std::string parse(std::string const& full, std::string const& qry, size_t& pos) {
  ...
  if (pos != std::string::npos) {
    ...
    throw std::invalid_argument{std::string("Couldn't parse model file! (can't find '") + qry +
                                "'!)"};
  }
  return result;
}

static std::string parse_between(std::string const& full, std::string const& lhs,
                                 std::string const& rhs, size_t& pos) {
  std::string result;
  ...
  if (rhs_begin == std::string::npos) {
    throw std::invalid_argument{std::string("Couldn't parse model file! (can't find '") + rhs +
                                "'!)"};
  } else if (rhs_begin == 0) {
    throw std::runtime_error{std::string("Nothing inbetween '") + rhs + "' and '" + lhs + "'?"};
  } else {
...
static std::string from_raxml_8(const std::string& file) {
  ...
  std::ifstream t(file);
  std::string full((std::istreambuf_iterator<char>(t)), std::istreambuf_iterator<char>());
  size_t pos = 0;
  ...
  for (size_t i = 0; i < chars.length() - 1u; ++i) {
    for (size_t k = i + 1u; k < chars.length(); ++k) {
      ...
      rates += parse(full, std::string("rate ") + chars[i] + " <-> " + chars[k] + ": ", pos);
    }
  }
  rates += "}";
  ...
  for (size_t i = 0; i < chars.length(); ++i) {
    ...
    freqs += parse(full, std::string("freq pi(") + chars[i] + "): ", pos);
  }
  freqs += "}";
...
static std::string from_iqtree(const std::string& file) {
  ...
  for (size_t i = 0; i < chars.length() - 1u; ++i) {
    for (size_t k = i + 1u; k < chars.length(); ++k) {
      ...
      rates += parse(full, std::string() + chars[i] + "-" + chars[k] + ": ", pos);
    }
  }
  rates += "}";
  model_desc.append(rates);
  ...
  for (size_t i = 0; i < chars.length(); ++i) {
    ...
    freqs += parse(full, std::string("pi(") + chars[i] + ") = ", pos);
  }
  freqs += "}";
  model_desc.append(freqs);
...
enum class ModelfileType { kRaxml8, kRaxmlng, kIqtree, kUnknown };
