...
static void write_subset(genesis::sequence::SequenceSet const& set,
  ...
  if (not subset_labels.empty()) {
    throw std::invalid_argument{
        "Could not find all references in the query file to produce an appropriate "
        "reference.fasta!"};
  }
...
void split(std::string const& ref_msa, std::vector<std::string> query_files,
  ...
  for (auto const& qry_file : query_files) {
    ...
    if (qry_width and cur_qry_width != qry_width) {
      throw std::invalid_argument{std::string("Query file '") + qry_file +
                                  "' appears to have an alignment width" +
    ...
    if (cur_qry_width != ref_width) {
      LOG_WARN << "The query alignment file '" << qry_file
               ...
               << "This is likely due to the alignment tool stripping gap-only columns, or adding "
                  "columns to the reference."
