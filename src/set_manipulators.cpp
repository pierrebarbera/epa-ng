  ...
  if (not src.empty()) {
    ...
    for (auto it : src) {
      ...
      if (prev_branch_id != branch_id) {
        dest[branch_id];
        dest[branch_id].insert(dest.at(branch_id).end(), src.at(branch_id).begin(),
...
void merge(Timer<>& dest, const Timer<>& src) { dest.insert(dest.end(), src.begin(), src.end()); }
...
#pragma omp parallel for schedule(dynamic)
  ...
  for (size_t j = 0; j < sample.size(); ++j) {
    ...
    // find the maximum
    auto max =
        std::max_element(pq.begin(), pq.end(), [](const Placement& lhs, const Placement& rhs) {
...
void sort_by_logl(PQuery<Placement>& pq) {
  std::sort(pq.begin(), pq.end(), [](const Placement& lhs, const Placement& rhs) -> bool {
    return lhs.likelihood() > rhs.likelihood();
  });
}
...
pq_iter_t until_accumulated_reached(PQuery<Placement>& pq, const double thresh, const size_t min,
  ...
  for (pq_iter = pq.begin(); num_summed < max and sum < thresh; ++pq_iter, ++num_summed) {
    // sum up until threshold is passed. if we abort before it is passed, we would have the
    // possibility of empty lists
    sum += pq_iter->lwr();
  }
...
void discard_by_support_threshold(Sample<Placement>& sample, const double thresh, const size_t min,
                                  const size_t max) {
...
void filter(Sample<Placement>& sample, const Options& options) {
  if (options.acc_threshold) {
    LOG_DBG << "Filtering output by accumulated threshold: " << options.support_threshold
            << std::endl;
    discard_by_accumulated_threshold(sample, options.support_threshold, options.filter_min,
                                     options.filter_max);
  } else {
    LOG_DBG << "Filtering output placements below threshold: " << options.support_threshold
            << std::endl;
    discard_by_support_threshold(sample, options.support_threshold, options.filter_min,
                                 options.filter_max);
  }
}
...
void find_collapse_equal_sequences(MSA& msa) {
  ...
  for (auto target_iter = msa.begin(); target_iter != end_merge_range; target_iter++) {
    ...
    begin_merge_range = partition(target_iter + 1, end_merge_range,
                                  [&target](const Sequence& query) { return !(target == query); });
