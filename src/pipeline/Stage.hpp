...
public:
  ...
  virtual bool exec() const final { return exec_; }

  virtual void exec(const bool b) final { exec_ = b; }
...
public:
   ...
   */
  Typed_Stage(accept_func_type accept, process_func_type process, put_func_type put)
      : accept_(accept), process_(process), put_(put) {}
  ...
  Typed_Stage(process_func_type process)
      : accept_([](in_type) {}), process_(process), put_([](out_type) {}) {}

  Typed_Stage() = delete;
  ~Typed_Stage() = default;

  inline void accept(in_type& arg) const { accept_(arg); }

  inline out_type process(in_type& arg) const { return process_(arg); }

  inline void put(out_type& arg) const { put_(arg); }

  void set_accept_func(accept_func_type& f) { accept_ = f; }

  void set_put_func(put_func_type& f) { put_ = f; }
