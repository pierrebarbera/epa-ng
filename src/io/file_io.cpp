...
MSA build_MSA_from_file(const std::string& msa_file, const MSA_Info& info, const bool premasking) {
  MSA msa;
...
static rtree_mapper determine_edge_num_translation(pll_unode_t const* const vroot, bool const left,
  ...
  if (left) {
    ...
    // in this case the edge num in the utree that previously held the rtree root is the last branch
    // by definiton
  ...
  rtree_mapper mapper(utree_root_edge, rtree_proximal_edge, rtree_distal_edge, proximal_length,
                      distal_length, left);
...
pll_utree_s* build_tree_from_file(const std::string& tree_file, Tree_Numbers& nums,
                                  rtree_mapper& mapper, const bool preserve_rooting) {
  pll_utree_t* tree;
  pll_rtree_t* rtree;
  ...
  if ((tree = pll_utree_parse_newick(tree_file.c_str()))) {
    ...
    if ((tree == NULL) or (tree == PLL_FAILURE)) {
      throw std::runtime_error{std::string("pll_rtree_unroot failed with message: ") + pll_errmsg +
                               " (" + std::to_string(pll_errno) + ") "};
    }
    ...
    if (left) {
      /* if the virtual root sits on the left subtree we need to correct for an inconistent way the
       * pll_rtee_unroot function returns the root node. It returns the node whose ->back is the
       * right hand subtree (from the rooted perspective), which would be wrong as this is the
       * subtree that post and pre order traversals recurse into first.
       */
    ...
    // get the translation
    if (preserve_rooting) {
      mapper = determine_edge_num_translation(tree->vroot, left, proximal_length, distal_length);
      if (rtree->root->label) {
...
pll_partition_t* make_partition(const raxml::Model& model, Tree_Numbers& nums, const int num_sites,
  ...
  auto partition = pll_partition_create(
      ...
      model.num_states(), num_sites, 1, nums.branches, model.num_ratecats(),
      (nums.inner_nodes * 3) + nums.tip_nodes, /* number of scaler buffers */
      attributes);

  if (not partition) {
    throw std::runtime_error{
        std::string("Could not create partition (make_partition). pll_errmsg: ") + pll_errmsg};
  }
