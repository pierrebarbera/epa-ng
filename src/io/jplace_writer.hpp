...
public:
  ...
  jplace_writer(const std::string& out_dir, const std::string& file_name,
                const std::string& tree_string, const std::string& invocation_string,
                rtree_mapper const& mapper)
      : tree_string_(tree_string), invocation_(invocation_string), mapper_(mapper) {
    init_mpi_();
...
// finalize and close
    ...
    if (local_rank_ == 0) {
      ...
      MPI_File_write(shared_file_, trailing.str().c_str(), trailing.str().size(), MPI_CHAR,
                     MPI_STATUS_IGNORE);
    }
  ...
  void write(Sample<>& chunk) {
    ...
    prev_gather_ =
        std::async(std::launch::async, [chunk = chunk, this]() mutable { this->write_(chunk); });
#else
    write_(chunk);
#endif
  }
...
protected:
  void write_(Sample<>& chunk) {
    ...
    if (shared_file_) {
      ...
      MPI_File_write_at_all(shared_file_, displacement, buffer_str.c_str(), buffer_str.size(),
  ...
  virtual void init_file_(const std::string& out_dir, const std::string& file_name) {
    const auto file_path = out_dir + file_name;
#ifdef __MPI
    MPI_File_open(MPI_COMM_WORLD, file_path.c_str(), MPI_MODE_WRONLY | MPI_MODE_CREATE,
                  MPI_INFO_NULL, &shared_file_);
#else
